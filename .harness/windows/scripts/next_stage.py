...
# ============================================================================
#                      环境变量加载（在 ENABLE_AUTO_VALIDATION 使用前）
# ============================================================================
...
def run_validation_task(task_id, task):
    ...
    # 获取执行命令
    command = AUTO_EXECUTE_COMMANDS.get(category)
    if not command:
        warning(f"未定义 category '{category}' 的自动执行命令", file=sys.stderr)
        return False
...
# 自动执行的 category（会跳过 Dev 阶段，直接运行命令）
# 注意：不要添加 'validation'，这会导致任务被跳过！
AUTO_EXECUTE_CATEGORIES = ['test', 'style']

# 自动执行的命令映射
AUTO_EXECUTE_COMMANDS = {
    'test': 'php8 artisan test',           # 自动运行测试
    'style': './vendor/bin/pint --test',    # 自动运行代码风格检查
    # 注意：不要添加 'validation' 的命令映射！
}

# 是否启用验证任务自动执行（对 test 和 style 类任务生效）
    ...
    if stage == 'dev':
        ...
        if 'stages' in task and task['stages']['test'].get('test_results'):
            ...
            for test_name, result in task['stages']['test']['test_results'].items():
                # Skip non-dict values (like 'issues' array)
