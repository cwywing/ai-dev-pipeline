...
# -*- coding: utf-8 -*-
"""
Task Data Codec - 任务数据编码/解码器 (Windows 版本)
...
class TaskCodec:
    ...
    def get_task_json_path(cls) -> Path:
        """
        获取 task-index.json 路径（向后兼容）

        注意：此方法使用脚本文件的父目录的父目录的父目录（即 .harness 目录）
        """
        # 使用脚本文件路径向上三级获取 .harness 目录
        # scripts 目录 -> windows 目录 -> .harness 目录
        script_dir = Path(__file__).parent  # scripts 目录
        windows_dir = script_dir.parent  # windows 目录
        harness_dir = windows_dir.parent  # .harness 目录
        return (harness_dir / "task-index.json").resolve()
