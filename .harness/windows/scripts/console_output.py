...
def _safe_print(message: str, file: TextIO = sys.stdout, no_emoji: bool = False) -> None:
    ...
    try:
        ...
        # 尝试直接写入文件对象
        # 使用 write() 而不是 print() 避免额外的编码处理
        file.write(clean_msg + '\n')
        file.flush()
    except UnicodeEncodeError as e:
        # GBK 编码失败，移除所有非 GBK 兼容字符
        try:
            # 尝试 GBK 编码，失败则用 ASCII
            try:
                clean_msg.encode('gbk')
                file.write(clean_msg + '\n')
            except UnicodeEncodeError:
                # 只保留 ASCII 字符
                ascii_msg = clean_msg.encode('ascii', errors='ignore').decode('ascii')
                file.write(ascii_msg + '\n')
            file.flush()
        except Exception:
            # 最后的兜底：只打印简单消息
            file.write("[OUTPUT_ERROR] Could not output message\n")
            file.flush()
